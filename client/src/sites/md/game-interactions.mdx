...
Damit ein Spiel auch Spass macht, soll es interaktiv sein. Das heisst der
...
Oftmals ist es in Spielen so dass ein Spieler Lebenspunkte hat. Wenn er in einen
...
Das meiste wird auch schon wieder von der Klasse `Base2DScene` gemacht, wir
müssen das ganze nur noch erweitern.

<Example title="Mit Blumen heilen">

Der Code kann genau so aus dem Level 1 übernommen werden und auf die Datein vom
Level 2 angepasst werden. Dann fügen wir ein paar neue Methoden ein, mit denen
können wir überschreiben was passiert wenn 2 Objekte zusammenstossen.

Überschreiben heisst das wir die Funktionalität aus der Basisklasse komplett
...
Das ist eigentlich schon alles was wir machen müssen. Hier wäre es aber besser
...
### Eigenschaften von Spielobjekten verändern

Wie oben bereits erwähnt möchten wir die Eigenschaften von einem Spielobjekt
...
Methode `heal` geben, die können wir dann von aussen aufrufen. Dann kann sich
...
selber macht, können wir die Logik einfach anpassen, und es ist für alle gleich.

<Example title="Heilfunktion auf dem Spieler">

Damit wir den Spieler heilen können, brauchen wir zuerst die Eigenschaft `hp`
oder was ähnliches auf dem Spieler. Damit wir nicht zu viel heilen, brauchen wir
auch sowas wie `maxHp`. Der Code unten zeigt wie Sie die Eigenschaften auf dem
Spieler erstellen können, und diese in der `heal` Methode verwenden.

Wir erweitern dazu den Spieler aus dem letzten Abschnitt, Sie brauchen also
nicht allen Code zu kopieren, sondern müssen schauen wo die neuen Stücke
hinkommen, und welche neu sind.
...
Sehr ähnlich können wir jetzt auch mit Schaden umgehen, oder anderen Dingen wie
...
Lebenspunkte des Spieler verändern darf.
